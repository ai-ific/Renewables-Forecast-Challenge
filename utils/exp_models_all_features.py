...
if __name__ == "__main__":
    ...
    '''
    10th Experiment. Model with all the features
    ...
    time_steps = 3
    n_units = 12
    selected_columns = ['Hour', 'Day', 'Temperature', 'Dew Point', 'Humidity', 
                        'Pressure', 'Speed', 'Gust', 'Pressure', 
                        'Precip. Rate.', 'Precip. Accum.','Demanda (W)']
    ...
    rmse_df = deep_learning.run_experiment(model, Xtr, Ytr, Xte, Yte,
                   ...
                   scaler = scaler, 
                   verbose = True)
    result_filenames.append('results/model_allf_12_shuffle.csv')
    rmse_df.to_csv(result_filenames[-1], index=False)
    
    '''
    11th Experiment. Model with all the features II
    Input: Energy demand of 3 previous hours to hour t
    Output: Energy demand of hour t
    ...
    rmse_df = deep_learning.run_experiment(model, Xtr, Ytr, Xte, Yte,
                   ...
                   scaler = scaler, 
                   verbose = True)
    result_filenames.append('results/model_allf_50_shuffle.csv')
    rmse_df.to_csv(result_filenames[-1], index=False)
    
    '''
    12th Experiment. Model with all the features III
    Input: Energy demand of 3 previous hours to hour t
    Output: Energy demand of hour t
    ...
    rmse_df = deep_learning.run_experiment(model, Xtr, Ytr, Xte, Yte,
                   ...
                   scaler = scaler, 
                   verbose = True)
    result_filenames.append('results/model_allf_50_10_shuffle.csv')
    rmse_df.to_csv(result_filenames[-1], index=False)
